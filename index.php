...
if (isset($_SESSION['user_id'])) {
    ...
    // --- BỔ SUNG LOGIC TÍNH THU/CHI THÁNG NÀY ---
    $current_month = date('m');
    $current_year = date('Y');

    // Tính tổng Thu nhập tháng này
    $sql_income = "SELECT SUM(t.amount) as total FROM transactions t 
                   JOIN categories c ON t.category_id = c.id 
                   WHERE t.user_id = $user_id AND c.type = 'income' 
                   AND MONTH(t.transaction_date) = $current_month AND YEAR(t.transaction_date) = $current_year";
    $res_income = $conn->query($sql_income);
    $row_income = $res_income->fetch_assoc();
    $total_income = $row_income['total'] ? $row_income['total'] : 0;

    // Tính tổng Chi tiêu tháng này
    $sql_expense = "SELECT SUM(t.amount) as total FROM transactions t 
                    JOIN categories c ON t.category_id = c.id 
                    WHERE t.user_id = $user_id AND c.type = 'expense' 
                    AND MONTH(t.transaction_date) = $current_month AND YEAR(t.transaction_date) = $current_year";
    $res_expense = $conn->query($sql_expense);
    $row_expense = $res_expense->fetch_assoc();
    $total_expense = $row_expense['total'] ? $row_expense['total'] : 0;
?>
    <style>
        .dashboard-container {
            display: flex;
    ...
    <div class="dashboard-container">
        ...
        <div class="card">
            ...
            <div class="money" style="color: #2ecc71;">+ <?php echo number_format($total_income); ?> đ</div>
            <small>Tháng <?php echo $current_month . '/' . $current_year; ?></small>
        </div>
        <div class="card">
            <h3>Đã chi tiêu tháng này</h3>
            <div class="money" style="color: #ed4956;">- <?php echo number_format($total_expense); ?> đ</div>
            <small>Tháng <?php echo $current_month . '/' . $current_year; ?></small>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div style="display: flex; gap: 10px; margin-top: 15px; flex-wrap: wrap;">
            <a href="modules/transactions/user_transactions/user_add.php" style="background: #0095f6; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px;">+ Thêm Giao dịch</a>
